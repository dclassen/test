...
podTemplate(label: 'jenkins-pipeline', containers: [
    containerTemplate(name: 'jnlp', image: 'jenkinsci/jnlp-slave:2.62', args: '${computer.jnlpmac} ${computer.name}', workingDir: '/home/jenkins', resourceRequestCpu: '200m', resourceLimitCpu: '200m', resourceRequestMemory: '512Mi', resourceLimitMemory: '512Mi'),
    ...
    containerTemplate(name: 'kubectl', image: 'lachlanevenson/k8s-kubectl:v1.7.6', command: 'cat', ttyEnabled: true),
    containerTemplate(name: 'scala-sbt', image: 'hseeberger/scala-sbt:latest', command: 'cat', ttyEnabled: true)
],
volumes:[
      ...
      stage ('deploy to k8s') {
        container('helm') {
          ...
          pipeline.helmDeploy(
            dry_run       : false,
            name          : config.app.name,
            namespace     : config.app.namenamespace,
            version_tag   : image_tags_list.get(0),
            chart_dir     : chart_dir,
            replicas      : config.app.replicas,
            cpu           : config.app.cpu,
